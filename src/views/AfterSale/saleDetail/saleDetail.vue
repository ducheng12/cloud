  ...
  <div class="app-container">
    ...
    <div class="primary">
      ...
      <div class="primary-list" style="padding-top: 12px">
        <div>退款单号:</div>
        <div>{{ allData.retreatId }}</div>
      </div>
      <div class="primary-list">
        <div>状态:</div>
        <div>{{ allData.retreatStatusValue }}</div>
      </div>
      <div class="primary-list">
        <div>云店名称:</div>
        <div>{{ allData.storeName }}</div>
        <div v-if="allData.storeName" class="lookInfo" @click="detailCloud(allData.storeId)">查看详情</div>
      </div>
      <div class="primary-list">
        <div>云店账号:</div>
        <div>{{ allData.storeAccount }}</div>
      </div>
      <div class="primary-list">
        <div>退货运单号:</div>
      ...
      <div class="primary-list">
        <div>退货收货人信息:</div>
        <div>{{ allData.relation }}</div>
        <div style="margin-left:14px;color:rgba(0,0,0,0.45);">{{ allData.relationPhone }}</div>
        <div style="margin-left:14px;color:rgba(0,0,0,0.45);">{{ allData.address }}</div>
      </div>
      <div class="primary-list">
        <div>退货时间:</div>
        <div>{{ allData.returnTime }}</div>
      </div>
      <div class="primary-list">
        <div>退款时间:</div>
        <div>{{ allData.refundTime }}</div>
      </div>
    </div>
    <!--  -->
    <div class="primary">
      ...
      <div class="primary-list">
        <div>供应商名称:</div>
        <div>{{ allData.supplierName }}</div>
        <div v-if="allData.supplierName" class="lookInfo" @click="detaiSupplier(allData.supplierId)">查看详情</div>
      </div>
      <div class="primary-list">
        <div>供应商账号:</div>
        <div>{{ allData.supplierAccount }}</div>
      </div>
      <div class="primary-list">
        <div>云店名称:</div>
        <div>{{ allData.storeName }}</div>
    ...
    <div class="primary">
      <div class="primary-title">平台介入详情</div>
      <div class="primary-list" style="padding-top: 12px">
        <div>操作人:</div>
        <div>{{ allData.operator }}</div>
      </div>
      <div class="primary-list" style="padding-top: 12px">
        <div>操作时间:</div>
        <div>{{ allData.operationTime }}</div>
      </div>
      <div class="primary-list" style="padding-top: 12px">
        <div>处理结果:</div>
        <div>{{ allData.handleResult }}</div>
      </div>
    </div>
    <!--  -->
    <div class="primary">
...
export default {
  ...
  methods: {
    ...
    getData() {
      const that = this
      const params = {
        retreatId: this.$route.query.retreatId
      }
      Basic.general('after_sale/selectAfterSaleInfo', params).then(res => {
        const { data } = res
        that.allData = data
