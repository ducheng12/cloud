  ...
  <div>
    ...
    <el-form ref="rulesForm" class="Mssg_form" size="small" label-position="top">
      ...
      <div class="basic-mssg">
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item :required="true" class="fontWeig" label="云店名称">
              <el-input v-model="baseInfoForm.shopName" style="width:260px;" placeholder="请输入云店全称" maxlength="60"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label-width="110" :required="true" class="my_form-item fontWeig" label="云店简称">
              <el-input v-model="baseInfoForm.merchBasicInfo.shortName" style="width:260px" placeholder="请输入云店简称" maxlength="20"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label-width="200" class="fontWeig" label="主营类目" :required="true">
              <el-select v-model="baseInfoForm.mainType" style="width:260px;" placeholder="请选择">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label-width="72" :required="true" label="详细地址" class="my_form-item fontWeig">
              <el-input v-model="baseInfoForm.merchBasicInfo.address" style="width:260px" maxlength="100" placeholder="请输入云店详细地址"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-row :gutter="8">
          <el-col :span="8">
            <el-form-item label-width="72" :required="true" label="业务联系电话" class="my_form-item fontWeig">
              ...
              ></el-input>
              <div>该手机号作为懂衣云店小程序登录账号，请正确填写</div>
            </el-form-item>
          </el-col>
        </el-row>
      </div>
      ...
      <!-- 云店信息 -->
      <el-form-item label-width="0px" class="label-title">
        <span style=" color: black;">云店信息</span>
      </el-form-item>
      <div class="basic-mssg">
        <el-row :gutter="10">
          <el-col :span="8">
          ...
          <el-col :span="8">
            <el-form-item label="收银台照片" class="fontWeig" style="display: inline-block;" :required="true">
              <MultipleUpload :limit="1" :image-list="CashierList" @handle-upload="UploadCashierPic" />
              <div style="margin-top:10px;">请上传收银台照片，仅支持jpg或png格式</div>
            </el-form-item>
          </el-col>
        </el-row>
      </div>
      ...
      <div class="basic-mssg">
        <el-row :gutter="20">
          ...
          <el-col v-if="baseInfoForm.shopId" :span="8">
            <el-form-item label-width="72" label="入驻审核状态" class="my_form-item fontWeig">
              <el-tag v-if="submitStatus === 0" type="info">平台已驳回</el-tag>
              <el-tag v-if="submitStatus === 1" type="info">待银行审核</el-tag>
              <el-tag v-if="submitStatus === 2" type="success">银行审核通过</el-tag>
              <el-tag v-if="submitStatus === 3" type="danger">银行审核驳回</el-tag>
              <el-tag v-if="submitStatus === 5" type="danger">待平台审核</el-tag>
            </el-form-item>
          </el-col>
        </el-row>
      </div>
      <el-form-item style="text-align:center">
        ...
        <el-button v-if="submitStatus === 0 || submitStatus === 3 || submitStatus === 5 || submitStatus === '' " style="width:65px;" size="small" type="primary" @click="handleConfirm">保 存</el-button>
        <el-button style="width:65px;" size="small" type="primary" @click="$router.back()">{{ submitStatus === 1 || submitStatus === 2 ? '返 回' : '取 消' }}</el-button>
        <!-- <el-button v-else style="width:65px;margin-left: 72px;" type="primary" size="small" @click="handleConfirm">提交审核</el-button> -->
        <!-- <el-button v-else style="width:65px;margin-left: 72px;" type="primary" size="small" @click="handleConfirm">提交审核</el-button> -->
      </el-form-item>
    </el-form>
  </div>
...
export default {
  ...
  created() {
    const vm = this
    vm.creatorName = this.$store.state.user.name
    vm.cityList = cityData()
    if (vm.$route.query.id) {
      vm.queryGoods(vm.$route.query.id)
    }
  },
  methods: {
    // 身份证正面上传成功
    ...
    // 收银台照片上传成功
    UploadCashierPic(val) {
      const vm = this
      vm.baseInfoForm.merchFileInfo.cashierPic = val.map(t => t.fileUrl).join(',')
    },
    // 门店详情
    async queryGoods(id) {
      const params = {
      ...
      const vm = this
      await Basic.general('shop/detail', params).then(res => {
        // 回显数据
        const {
          merchantSettleInfo, createUser, contactName, contactPhone, shopName,
          provinceCode, cityCode, districtCode, mainType, shopId, verifyStatus } = res.data
        this.baseInfoForm.shopId = shopId
        ...
        vm.baseInfoForm.shopName = shopName // 云店名称
        vm.baseInfoForm.provinceCode = provinceCode // 省code
        vm.baseInfoForm.cityCode = cityCode // 市code
        vm.baseInfoForm.districtCode = districtCode // 区code
        vm.region = [provinceCode, cityCode, districtCode] // 回显地区
        vm.baseInfoForm.mainType = mainType + '' // 类目id
        vm.submitStatus = verifyStatus
        // 商户资质资料
        ...
        vm.baseInfoForm.merchFileInfo.cashierPic = merchantSettleInfo.merchFileInfo.cashierPic // 收银台执照
        // 回显图片
        ...
        vm.CashierList.push(merchantSettleInfo.merchFileInfo.cashierPic) // 营业执照图片回显
        // 商户结算卡资料
    ...
    getVerify() {
      const vm = this
      const {
        merchBasicInfo, mainType, contactPhone, merchSettInfo, merchFileInfo,
        merchContactInfo, merchAptitudeInfo, shopName } = vm.baseInfoForm
      if (!shopName) {
        vm.$message.warning('云店名称不能为空！')
        return false
      }
      if (!merchBasicInfo.shortName) {
        vm.$message.warning('云店简称不能空！')
        return false
      }
      if (!mainType) {
      ...
      if (!merchFileInfo.cashierPic) {
        vm.$message.warning('请上传收银台照片！')
        return false
      }
      return true
    },
    ...
    handleConfirm() {
      ...
      console.log(vm.baseInfoForm.merchAptitudeInfo.legalPerson)
      if (this.$route.query.id) {
        ...
        Basic.normal('shop/create', this.baseInfoForm).then(res => {
    ...
    handleCancel() {
      const vm = this
      const {
        merchBasicInfo, mainType, contactPhone, shopName } = vm.baseInfoForm
      if (!shopName) {
        vm.$message.warning('云店名称不能为空！')
        return false
      }
...
/deep/ .el-form-item__label{
  ...
  .AddTitle /deep/ .el-page-header__left::after{
    width:0;
  }
  .AddTitle /deep/  .el-page-header__title{
  font-size: 20px;
}
.Mssg_form{
  padding: 20px 48px;
